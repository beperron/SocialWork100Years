...
It should be noted that the second-level search was specified to be overly inclusive to help ensure the most comprehensive search possible.  All journal titles from the secondary search are identified in the following summary.  

library(dplyr)
temp <- bwr.df[bwr.df$articleID == 28300,]
27743 and 28300.
...
# Re-write this to call the R-object directly
# Overview of processed datafile

Provided below are some basic descriptive statistics before performing any data cleaning.

```{r echo=FALSE, comment=NA} 
number.of.articles <- length(which(bwr.df$attributes == "TI"))
range.of.years <- range(bwr.df$record[bwr.df$attributes == "YR"])
