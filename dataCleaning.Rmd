...
barplot(diff(nsf$n.grads), ylab="Change in value", main="Growth")
...
It is estimated that 90% of all data in human history has been created in the last two years -- roughly 2.5 quintillion bytes of data every day [IBM, nd-a](http://www-01.ibm.com/software/data/bigdata/what-is-big-data.html).  _Big data_ has become ubiquitious in the business community and has recently captured the attention of governmental organizations given its potential to generate actionable insights from a wide range of data sources [IBM, nd-b](http://www-01.ibm.com/software/data/bigdata/industry-government.html).  The concept is most commonly understood with respect to _volume_ -- that is, an amount of data that exceeds the storage capacity of conventional database systems.  However, _big data_ is also distinguished from traditional data with respect to _velocity_ and _variety_.  Velocity refers to data arriving at a rate that exceeds conventional computing power, and variety refers to data structures that are not directly amenable to analysis with conventional software packages.  Volume, velocity and variety are commonly referred to as the three _V's_.  

Big data comes from 


Although the popularity of In fact, the interest in big data is growing in popularity as an internet search term, while ``social work'' is experiencing a decline.  

(but not limited to) sensors that capture events in real time, GPS, social media sites, digitized text, electronic audio and video, purchase transactions, and weblogs.  These data provide rich opportunities for social work research that are  
