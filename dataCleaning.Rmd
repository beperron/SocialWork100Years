...
library(stringi)
countries.extracted <- na.omit(within(LO.df2, text1<- stri_extract(LO.df2$record, opts_fixed=TRUE, regex=indx.country)))[-2]
...
GDP<-select(GDP, Country=Country.Name, GDPperCapita=GDPperCapita2013, GDPRank)


PPP<-read.csv("PPP.csv")
PPP$PPP2013<-gsub(",", "", PPP$PPP2013)
PPP$PPP2013<-as.character(PPP$PPP2013)
PPP$PPP2013<-as.numeric(PPP$PPP2013)

articleCountries<-table(locations$record)
articleCountries<-as.data.frame(articleCountries)
articleCountries<-articleCountries%>%
                    select(Country=Var1, articleCount=Freq)

rankSet<-merge(articleCountries, GDP, by="Country", all=TRUE)
rankSet$articleCount[is.na(rankSet$articleCount)]<-0
rankSet<-merge(rankSet, PPP, by="Country", all=TRUE)

rankSet<-mutate(rankSet, articleRank=rank(-articleCount))
```

```{r worldmap, eval=FALSE}
...
```{r pages, eval=FALSE}
pages.df <- full.df %>% filter(attributes == "pages")
page.identifier <- grepl("[-p]", pages.df$record)
pages.df.reduced <- pages.df[page.identifier == FALSE,]

reduced <- full.df[full.df$articleID %in% pages.df.reduced$articleID, ]
reduced <- filter(reduced, attributes == "article" | attributes == "journal" | 
            attributes == "pubYear" | attributes == "pages" | attributes == "abstract")
...
```{r}
mapCountryData(spdf, nameColumnToPlot="frequency", catMethod="quantiles", 
                                  colourPalette="white2Black", 
                                  mapTitle="Article Counts by Country, 1989-2013")
```


\newpage
```{r}
mapCountryData(spdf2, nameColumnToPlot="frequency", catMethod="quantiles", 
                    colourPalette="white2Black", 
                    mapTitle="Article Counts for Countries with Sustained Presence, 1989-2013")
