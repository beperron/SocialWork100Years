...
```{r appendData, comment=NA, echo=FALSE, eval=TRUE}
appendData.f()
...
```{r shortArticles, comment=NA, echo=FALSE, warning=FALSE, eval=TRUE}
shortArticles.f()
```

```{r titleMatching, echo=FALSE, eval=TRUE}
titleMatching.f()
...
```{r otherDocuments, echo=FALSE, eval=TRUE}
otherDocuments.f()
...
```{r final, comment=NA, echo=FALSE, eval=TRUE}
fewArticles.f()
...
```{r finalList, comment=NA, echo=FALSE, eval=TRUE}
unique.titles <- filter(full.df, attributes == "journal") %>%
    group_by(record) %>%
    summarise(N = n())
...
```{r locationExtraction2, eval=TRUE}
...
```{r plotLocationWithUS, eval=TRUE}
