...
The articles were reviewed and a first round of exclusions were made.  
...
exclusions <- c(
    ...
    "University of Chicago Social Service Monograph")
ebsco.remove <- full.df[full.df$record %in% exclusions, ]
ebsco.remove <- ebsco.remove[, "articleID"]
full.df <- full.df[!(full.df$articleID %in% ebsco.remove),]
...
```{r}
full.df$record <- ifelse(full.df$record == "Eugenics and Social Welfare Bulletin", 
...
unique.titles <- filter(full.df, attributes == "SO") %>%
    group_by(record) %>%
    summarise(
        N = n())
```
...
n.so.yr <- filter(full.df, attributes == "SO" | attributes == "YR")

n.so <- filter(full.df, attributes == "SO") %>% mutate(title = record) %>% 
        select(-attributes, -record)

n.yr <- filter(full.df, attributes == "YR") %>% mutate(year = record ) %>% 
        select(-attributes, -record)
...
n.articles.year <- filter(full.df, attributes == "YR") 
...
kp.df <- filter(full.df, attributes == "KP")
...
kp.yr <- filter(full.df, attributes == "YR") %>%
...
LO.df <- filter(full.df, attributes == "LO") %>%
         group_by(record) %>%
...
year <- filter(full.df, attributes == "YR") %>%
    mutate(year = as.numeric(record)) %>% select(-record, -attributes)

location.temp <- full.df %>%
