...
plot.article.count <- ggplot(articles.year.df, aes(as.factor(years), 
    ...
    annotate("rect", xmin = 2008-1989+1, xmax =  2012-1989+1, 
             ymin = 1700, ymax = 2050, alpha = .20) +
      annotate("rect", xmin = 2005-1989+1, xmax =  2007-1989+1, 
             ymin = 1400, ymax = 1750, alpha = .20)  
```
...
plot.journal.count <- ggplot(n.journals.year, aes(as.factor(record.y), 
    ...
    scale_y_continuous(breaks = c(seq(0, 100, 10)), limits = c(20, 70)) +
     annotate("rect", xmin = 2008-1989+1, xmax =  2012-1989+1, 
             ymin = 55, ymax = 70, alpha = .20) +
         annotate("rect", xmin = 2005-1989+1, xmax =  2007-1989+1, 
             ymin = 53, ymax = 62, alpha = .20)  
```
...
plot.author.count <- ggplot(n_authors, aes(as.factor(pubYear), y=average.n, group=1)) +  
    geom_ribbon(aes(ymin = average.n-std.dev, ymax=average.n+std.dev), colour="lightblue", fill="lightblue", alpha=.40) +
    geom_line(colour="black") + 
    ylab("Authors") +
    xlab("Year") +
    ggtitle("Average Number of Authors per Article by Year") +
...
year.location.df.noUS <- group_by(year.location.df, country, pubYear) %>%
                    summarise(N = n())
...
```{r plotLocationWithUS}
plot.location.withUS <- ggplot(year.location.df, aes(pubYear, N, group = country, colour=country)) + 
    geom_line() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), title = element_text(size = 10)) +
...
```{r twoyearoffset}
nsf.reduced <- nsf[1:10,]
nsf.reduced$years <- 2004:2013
