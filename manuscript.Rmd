...
plot.article.count <- ggplot(articles.df, aes(Year, N, group =1)) + 
    ...
    xlab("Year") + 
    ylab("") + 
    ggtitle("Article Count") +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) +
        scale_y_continuous(breaks = c(seq(0, 2200, 250)), limits= c(500, 2100)) 

    
plot.article.cumulative <- ggplot(articles.df, aes(Year, CumSum, group =1)) + 
    geom_line(colour=cp[2], size = 1) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8), 
    title = element_text(size = 10), plot.title=element_text(size=10)) + 
    xlab("Year") + 
    ylab("") + 
    ggtitle("Cumultative Article Count") +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) +
        scale_y_continuous(breaks = c(seq(0, 35000, 5000)), limits = c(0, 35000)) 
...
plot.journal.count <- ggplot(n.journals.year, aes(record.y, 
    ...
    xlab("Year") + 
    ylab("N") + 
    ggtitle("Journal Count") +
    scale_y_continuous(breaks = c(seq(0, 100, 5)), limits = c(20, 70)) +
     scale_x_discrete(breaks=seq(1989, 2015, 8)) 
...
journal.evidence.plot <- ggplot(na.omit(df1[df1$journal %in% df1.summarized$journal,]), aes(x = N, y= journal, group = journal, colour = studyType)) + 
...
pages.df2.melted <- filter(pages.df.melted, variable == "Mean.Pages")
...
Figure 1 uses a small multiples plot to summarize the number of article records
for each year throughout the study period.  According to Tufte (xxxx), small multiples (also referred to as
trellis and lattice charts) are the best design solution for visually enforcing
comparisons of change.  In the current useage, each journal is summarized with its own line plot using two different lines to represent the number of article records contained in the SWHD (orange line) and Scopus (purple line).  The journals are sorted in descending order based on the overall number of article records in the SWHD.  Journal titles are also abbreviated, and the full spellings are available in Appendix A.  These line plots help show publication patterns for each journal.  

The differences in publication counts are explained by a couple different reasons.  Scopus does not index journals prior to 1996, whereas the SWHD goes back to 1989.  Although the timeframe of the SWHD is longer than Scopus, the actual number of article records in the SWHD for many journals is lower than what is available in Scopus.  Scopus included editorials and book reviews in their article count, whereas these publications were specifically excluded from the SWHD.  Because Scopus data were available only in aggregate form, it was not possible to extract these data to make more refined comparisons.  In the construction of the SWHD, we also applied a series of cleaning procedures that further reduced the article count for many journals.  For example, we excluded article records that did not contain author names, an abstract or article title.  Again, because we were working only with aggregate data from Scopus, we were unable to apply the same cleaning procedures to the Scopus data.  And, finally, differences can also be attributed to gaps and errors in indexing in Scopus and all the databases used to construct the SWHD.  This is a known and longstanding problem with some social work journals (see Holden, Holden).  
