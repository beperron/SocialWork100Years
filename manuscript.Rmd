...
bigList <- left_join(n.so, n.yr) %>%
    group_by(title, year) %>%
    summarize(N = n())
bigList$shortTitle <- abbreviate(bigList$title)

bigList2 <- left_join(bigList, hIndex, by = "title")
bigList2 <- within(bigList2, quartile <- as.integer(cut(h5Index, quantile(h5Index, probs=0:4/4, na.rm=TRUE), include.lowest=TRUE)))
table(bigList2$h5Index, useNA = "always")

bigList2$quartileFactor <- as.factor(bigList2$quartile)
bigList2 <- bigList2 %>% arrange(desc(quartileFactor))


ggplot(bigList2, aes(year, N, group = 1, colour=quartileFactor)) + geom_line() + 
...
practiceGuidelines.df <- abstract.df[grep("practice\\sguidelines|treatment\\sguidelines?", abstract.df$abstract), ]
practiceGuidelines.df$studyType <- rep("pg", nrow(practiceGuidelines.df))

quasi.df <- abstract.df[grep("quasi\\s?experimental|non\\s?experimental", abstract.df$abstract), ]
quasi.df$studyType <- rep("quasi", nrow(quasi.df))

longitudinal.df <- abstract.df[grep("longitudinal\\sstudy", abstract.df$abstract), ]
longitudinal.df$studyType <- rep("longitudinal", nrow(longitudinal.df))

observational.df <- abstract.df[grep("cohort\\sstudy|case\\scontrol\\sstudy", abstract.df$abstract), ]
observational.df$studyType <- rep("observational", nrow(observational.df))

literatureReview.df <- abstract.df[grep("literature\\sreview|review of the.*literature", abstract.df$abstract),]
literatureReview.df$studyType <- rep("literatureReview", nrow(literatureReview.df))

survey.df <- abstract.df[grep("survey", abstract.df$abstract), ]
survey.df$studyType <- rep("survey", nrow(survey.df))

evidence.df <- rbind(ebp.df, meta.df, rct.df, systematicReview.df, 
     practiceGuidelines.df, quasi.df, longitudinal.df, observational.df, 
     literatureReview.df, survey.df)
...
evidence.summary.main <- filter(evidence.summary, studyType != "literatureReview" , studyType != "survey" ,
    studyType != "ebp" , studyType != "longitudinal", studyType != "pg", studyType != "quasi")



cp <- c("#fdb864", "#e66101", "#b2abd2", "#5e3c99" )
evidence.summary.main <- ungroup(evidence.summary.main)
evidence.summary.main$year <- as.numeric(evidence.summary.main$year)
evidence.summary.main$studyType <- as.factor(evidence.summary.main$studyType)
...
ggplot(as.data.frame(xtabs(N~year+studyType, evidence.summary.main)), 
    ...
    geom_area(alpha=.80, colour = "black", size = .2) + 
    scale_fill_manual(values=cp, 
          breaks = rev(levels(evidence.summary.main$studyType)),
            labels = c("Systematic review", "RCT", 
            "Cohort/Case Control", "Meta-analysis")) +
  ylim(c(0, 60)) + 
