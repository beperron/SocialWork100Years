...
ebp.df$ebp <- rep("ebp", nrow(ebp.df))

meta.df <- abstract.df[grep("meta\\s?analysis", abstract.df$abstract), ]
meta.df$meta <- rep("meta", nrow(meta.df))
...
rct.df$rct <- rep("rct", nrow(rct.df))

systematicReview.df <- abstract.df[grep("systematic\\s?review", abstract.df$abstract), ]
systematicReview.df$systematicReview <- rep("sr", nrow(systematicReview.df))

practiceGuidelines.df <- abstract.df[grep("practice\\sguidelines|treatment\\sguidelines?", abstract.df$abstract), ]
practiceGuidelines.df$practiceGuidelines <- rep("pg", nrow(practiceGuidelines.df))

quasi <- abstract.df[grep("quasi\\s?experimental|non\\s?experimental", abstract.df$abstract), ]
quasi.df$quasi <- rep("quasi", nrow(quasi.df))

longitudinal.df <- abstract.df[grep("longitudinal\\sstudy", abstract.df$abstract), ]
longitudinal.df$longitudinal <- rep("longitudinal", nrow(longitudinal.df))

observational.df <- abstract.df[grep("cohort\\sstudy|case\\scontrol\\sstudy", abstract.df$abstract), ]
observational.df$observational <- rep("observational", nrow(observational.df))

literatureReview.df <- abstract.df[grep("literature\\sreview|review of the.*literature", abstract.df$abstract),]
literatureReview.df$literatureReview <- rep("literatureReview", nrow(literatureReview.df))

survey.df <- abstract.df[grep("survey", abstract.df$abstract), ]
survey.df$survey <- rep("survey", nrow(survey.df))
