...
```{r}
articles.df$YearRecode <- c(nrow(articles.df):1)
ggplot(articles.df, aes(Year, N)) +  
  geom_smooth(method="lm") + 
  geom_line() +
  geom_point()

options(digits = 4)
x <- lm(N ~ YearRecode, data = articles.df)
summary(x)

articlesN <- rev(articles.df$N)
articlesN <- ts(articlesN, frequency=1, start = 1989, end = 2013)
diff(articlesN)/articlesN[-nrow(articlesN),]*100


curr <- articles.df$N[-1]
prev <- articles.df$N[1:(length(articles.df$N)-1)]
articles.df <- arrange(articles.df, Year)
articles.df$monChange <- c(0, (100 *  (curr-prev) / prev)) 
articles.df <- mutate(articles.df, positiveNegative = as.factor(ifelse(articles.df$monChange >= 0, "Positive", "Negative")))


ggplot(articles.df, aes(Year, monChange, fill = positiveNegative)) + geom_bar(stat="identity") 
mean(articles.df$monChange[2:25])
...
The advancement of social work as a profession is largely dependent upon its underlying knowledge base.  This knowledge base is built on a very large body of research that is primarily contained in social work journals and indexed in a variety of databases. As in other disciplines, the majority of the publication process, including the indexing of articles, has been handled by for-profit academic publishers.  While these publishers provide important services to the field, a number of unintended consequences have emerged. 

A very serious consequence that has received considerable attention is the expensive subscription fees associated with accessing the research produced by the field.  Universities face increased difficulties paying these fees while academic publishers are posting "higher profits than nearly any sector of commerce" (Schmitt, 2014).  In the absence of an academic affiliation, it is nearly impossible for individual social workers and social work agencies to have access to this research.  Open-access publishing has received attention as a potentially viable model to address this problem of subscription fees, but the model itself does not appear to have gained any traction. In fact, movement in the opposite direction has been observed.  For example, the _Journal of the Society for Social Work and Research_ (JSSWR) was established as an open-access journal in 2010, but converted to a subscription-based journal in 2014.      

Another consequence of this publication model that has received almost no attention is the absence of standardized systems and processes for organizing the research.  More specifically, every academic database provider uses different technologies for managing a subset of social work research defined by their contract.  Thus, we see variability in the user interfaces, query languages, and file structures. Effectively searching the literature necessitates an investment in identifying the various databases that house social work research, and then learning the different methods for querying and managing the search results in each.

Database aggregators (e.g., ProQuest and EbscoHost) and bibliography managers (e.g., EndNote and RefWorks) have helped address these problems.  However, comprehensive searches ultimately require the use of both database aggregators and stand-alone databases.  Even when search results are saved in a bibliography manager, a considerable amount of human intervention is required to ensure the meta-data are complete and in a standardized format.  From this perspective, it is not surprising that substantial errors have been identified in reference lists of social work journals (e.g., Spivey & Wilkis, 2004; Mitchell-Williams, Skipper, Alexander, & Wilkis, 2015).

Addressing the organizational problems of the social work research has benefits beyond simply making information retrieval more efficient and less prone to error.  For example, article meta-data can facilitate the assessment of scientific progress, identification of major scientific contributors, measurement of scientific output, journal impact, and measurement of productivity (e.g., xxxx).  A more complete source of meta-data can vastly improve this line of research, given that the quality of any given trend analysis is dependent upon the quality of its underlying data source.  It is also a necessary step for the development of decision support and recommender systems that can ultimately help social workers more effectively and efficiently address their information needs.  

The purpose of this study was to better understand the current organizational structure of the social work research by gathering together, cleaning, structuring, and analyzing all available meta-data for social work journal articles within a 25-year period (1989-2013).  This required the development of specialized software that we make available in an open source format.  Consistent with the philosophy of reproducible research, we also make all of our computer code for analyzing the data in an open format.
...
work journal titles using a variety of sources, including the book _An
...
To ensure comprehensive coverage, we supplemented our base list by searching the Internet and bibliogpraphic databases for journal titles that contained any of the following terms: 
...
by the study team to determine whether they should be included in the study as a disciplenary social work journal. When disagreements occurred, the authors reviewed the
...
Abstracts) and xx additional databases on ProQuest (e.g., ERIC, Sociological
...
sometimes used an ampersand (&). Other examples included inconsistent use of the
...
In addition to post-processing original article records, R was also used for all
other data analysis.  R is an ideal analytic tool because it is open-source and
has a suite of related tools for producing reproducible research.  More
specifically, all the source code is written using Rmarkdown -- an open-source
system that integrates data management, data analysis, and the written report. 
The source code and data extracts are also curated at openICPSR of the
Inter-university Consotrium for Political and Social Research of the University
of Michigan.  The source code is also available from the first author's GitHub
repository, where the code will be maintained and updated to support future
research projects. It should be noted that the full data files cannot be made
publicly available because these are considered proprietary data of the database
services.  However, to ensure full compliance with the best practices of
reproducible research (cite King), we have provided the journal editor with the
following conditions under which we would release the data: (1) the requester
must be affilitated with an insitution that provides access to both the
EbscoHost and ProQuest platforms along with the databases used in this study or
(2) hold a personal subscription to each platform and the databases utilized
here.  This is designed to meet the standards of maximum allowable data
transparency while respecting the proprietary nature of the data.

# Results


We collected a total of 3X,XXX article records from XX different databases.  Extensive cleaning procedures were applied to exclude documents that were not filtered in the original retrieval process.  More specifically, as described in the Methods section, we filtered documents to include only journals, while excluding editorials, book reviews, table of contents, and other documents that are not considered journal articles.  This reduced the total number of unique article records in the database to xx,xxxx.  

## Size and growth of social work
...
multiples plot shows the number of articles published each year for each journal
throughout the study time frame (orange line).  We also included data from
Scopus for comparative purposes (purple line).  The small multiples are sorted
in descending order based on the total number of article records in the HD.  The
sorting was done by row, so the overall plot so the overall plot is read from
left to right. Abbreviations are used in Figure 2, and the full journal name is
