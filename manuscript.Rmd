...
```{r objectMakerProQuest, warning=FALSE, message=FALSE, eval=FALSE, results='hide'}
# Clear workspace
rm(list=ls())
...
```{r objectMakerEbscoHost,warning=FALSE, message=FALSE, eval=FALSE, results='hide'}
# Clear workspace
rm(list=ls())
...
```{r cleaningFunctions, comment=NA, echo=FALSE, eval=TRUE}
# Read data files
...
```{r tableArticleSummary, eval=TRUE}
...
lab <- paste("N=", Overall.N$Overall.N, sep="")
xpos = rep(2002, 79)
ypos = rep(215, 79)
...
# Plotting function    
ggplot(sm.scopus.merged,  aes(Year, N)) + geom_line(colour="#e66101") +
    facet_wrap(~ Abbrev) + 
    geom_line(data = sm.scopus.merged, aes(Year, Documents), group = 1,colour="#5e3c99", alpha=.60) + 
    theme_bw() +
    theme(legend.position = "bottom", 
    panel.grid.minor = element_blank(), 
    axis.text.x = element_text(angle=45, hjust=1, size =8)) + 
    geom_text(data = ldata, aes(x =xpos, y = ypos, label = lab), size =3) + 
    ylim(c(0,225)) +
    scale_x_continuous(breaks=seq(1989, 2020, 8)) 
...
scopus.df <- read.csv("scopus.csv")
articles.df <-full_join(sm.df, scopus.df, by = c("Journal", "Year")) 
...
articles.df<- articles.df %>% 
    mutate(notCited = N - N * (X..Not.Cited/100))

articles.df<- articles.df %>% group_by(Year) %>%
    summarize(HD.N = sum(N, na.rm=TRUE), 
              Scopus.N = sum(Documents, na.rm=TRUE),
              Uncited.N = sum(notCited, na.rm=TRUE)) %>%
    mutate(HD.N = ifelse(HD.N == 0, NA, HD.N), Scopus.N = ifelse(Scopus.N == 0, NA, Scopus.N))

articles.df$Scopus.N[articles.df$Year > 2011] <- NA
articles.df$Uncited.N[articles.df$Year > 2011] <- NA
articles.df$Uncited.N <- ifelse(articles.df$Uncited.N == 0, NA, articles.df$Uncited.N)

articles.df.melted <- articles.df %>% 
    reshape2::melt(id.vars = "Year") 

#--------------------------------------------
#Create time series data to estimate uncited
#--------------------------------------------
    
uncited.ts <- as.vector(na.omit(articles.df$Uncited.N))
uncited.ts <- ts(uncited.ts, frequency = 16, start = c(1996,1))

foreward <- auto.arima(uncited.ts)
uncited.forecast <-data.frame(forecast(foreward, 2))
forecast.estimated <- uncited.forecast$Point.Forecast
forecast.years <- c(2012, 2013)


backcast <- rev(uncited.ts)
backcast <- auto.arima(backcast)
uncited.backcast <- data.frame(forecast(backcast, 7))
backcast.estimated <- rev(uncited.backcast$Point.Forecast)
backcast.years <- c(1989:1995)

Year <- c(backcast.years, forecast.years)
value <- c(backcast.estimated, forecast.estimated)
uncited.df <- data.frame(cbind(Year, value))
uncited.df$variable <- "Estimated"

percent.df <- rbind(articles.df.melted, uncited.df)
percent.df.cast <- reshape2::dcast(percent.df, Year  ~ variable, value.var = "value")
percent.df.cast$estimate.code <- ifelse(is.na(percent.df.cast$Uncited.N), 
    "arima.Estimated", ifelse(!is.na(percent.df.cast$Uncited.N), "Scopus.Estimated", percent.df.cast$Uncited.N))

percent.df.cast$percent <- ifelse(!is.na(percent.df.cast$Uncited.N), percent.df.cast$Uncited.N/percent.df.cast$HD.N, ifelse(!is.na(percent.df.cast$Estimated), percent.df.cast$Estimated/percent.df.cast$HD.N, percent.df.cast$Uncited.N))

percent.df.cast$NoCites <- ifelse(is.na(percent.df.cast$Uncited.N), percent.df.cast$Estimated, 
                           ifelse(is.na(percent.df.cast$Estimated), percent.df.cast$Uncited.N, percent.df.cast$Uncited.N))

percent.cast.a <- select(percent.df.cast, Year, HD.N, NoCites)
percent.cast.b <- select(percent.df.cast, Year, estimate.code, percent)

percent.melt.a <- reshape2::melt(percent.cast.a, id.vars="Year")
percent.melt.b <- reshape2::melt(percent.cast.b, id.vars=c("Year", "estimate.code"))



percent.melt.b <- rename(percent.melt.b, variable2 = variable, value2 = value)
percent.melt.df <- full_join(percent.melt.a, percent.melt.b, by = "Year")

percent.text <- select(percent.melt.df, Year, value, value2, variable) 
percent.text <- percent.text %>% filter(variable != "HD.N")
percent.text$value2 <- round(percent.text$value2, digits=3) *100 
percent.text$value2 <- paste(percent.text$value2, "%", sep="")
percent.text <- percent.text[c(1, 7, 11, 16, 19, 22, 24),]

#percent.df.cast <- select(percent.df.cast, Year, estimate.code, percent) %>% filter(Year <= 2013)
#colnames(percent.df.cast) <- c("Year", "variable", "value")



cp <- c("#5e3c99","#e66101")

ggplot(na.omit(subset(percent.melt.df, subset=variable != "Scopus.N")), 
    aes(Year, value, colour = variable)) + 
    geom_line() + 
    geom_point() +
    scale_color_manual(values=cp, labels = c("Number of Articles Published", "Number of Uncited Articles \n(Past 3 Years)")) +
    geom_text(data = percent.text, aes(Year, value+40, label = value2), size=4, colour="black") + 
    ylim(c(0, 2200)) + 
    theme(legend.position="bottom") +
...
articles.x <- as.factor(articles.year.df[c(1, 6, 12, 18, 25), 1]) 
articles.y <- articles.year.df[c(1, 6, 12, 18, 25), 2] 
articles.label <- as.character(articles.y)
articles.placement <- articles.y-50


plot.article.count <- ggplot(nsf, aes(as.factor(years), 
    y = n.grads, group=1)) + 
    ...
    ylim(c(0,2200)) + 
    geom_line(data = articles.year.df, aes(as.factor(years), y = year.count), colour="steelblue") + 
    #geom_text(data = nsf, aes(label = n.grads), vjust = 2, size = 2.75) +
    geom_point(data = articles.year.df, aes(as.factor(years), y = articles.year.df$year.count), colour="steelblue") + 
    annotate("text", factor(c(2002, 2005, 2008, 2012)), 
    c(165, 225, 250, 190) + 150, label = c("238","323", "338", "280") , size = 3) + 
    annotate("text", articles.x, articles.placement, label = articles.label, size=3)  


sm.scopus.merged.3 <- sm.scopus.merged %>% group_by(Year) %>%
    mutate(HD.N = sum(N)) 

sm.scopus.merged.4 <- sm.scopus.merged %>% select(Year, Documents)

    
sm.scopus.merged.4 <- na.omit(sm.scopus.merged.4)
sm.scopus.merged.4 <- sm.scopus.merged.4 %>% group_by(Year) %>% mutate(scopus.N = sum(Documents))

ggplot(sm.scopus.merged.3, aes(Year, HD.N), group=1) + geom_line() +
    geom_line(data =sm.scopus.merged.4, aes(Year, scopus.N), colour = "red") 
...
```{r plotH-Index, eval=TRUE}
final.list.index <- within(final.list.index, quartile <- as.integer(cut(h5Index, quantile(h5Index, probs=0:4/4, na.rm=TRUE), include.lowest=TRUE)))

final.list.index <- within(final.list.index, years <- as.numeric(last) - as.numeric(first) +1)

plotIndex <- ggplot(final.list.index, aes(x = n, y = h5Index )) + 
    geom_point(aes(colour=factor(quartile), size = years), alpha=.80) +
    geom_smooth() + 
...
n_authors2$year <- as.factor(c(1989:2013))


cp <- c("#fdb864", "#e66101", "#b2abd2", "#5e3c99" )

plot.author.count <- ggplot(n_authors2, aes(x = year, group=1)) + 
    geom_line(aes(y=Median, colour="Median"), size = .75, colour = "#fdb864", alpha = .70) + 
    geom_line(aes(y=Average, colour = "Average"), size = .75,  colour = "#e66101" ) +
    geom_point(aes(y=Average, size = Standard_Deviation), colour = "#e66101") +
...
```{r authorNames}
authorNames  <- filter(full.df, attributes == "author")

authorNames.df <- authorNames
authorNames.df$record <- sub("(?<=, [A-Z]).*$", "", authorNames.df$record, perl=TRUE)
authorNames.df <- authorNames.df %>% group_by(record) %>% 
    summarize(N = n()) %>%
    arrange(desc(N))
```{r gade, eval=TRUE}
years <- c(1989:2015)
...
year.labs <- as.character(seq(1989,2013, 2))  

ggplot(as.data.frame(xtabs(N~year+studyType, evidence.summary)), 
  ...
  ylim(c(0, 60)) + 
    scale_x_continuous(breaks = seq(0,24, 2), labels = year.labs) + 
    theme(axis.text.x = element_text(angle = 45, hjust=1), 
          panel.grid.minor=element_blank(),
...
ggplot(na.omit(df), aes(year, studyProp, colour = studyType, 
    group = studyType), na.rm=TRUE) + 
    geom_smooth(aes(fill=studyType), alpha = .60, colour = "dimgray") + facet_wrap(~studyType) + 
    scale_fill_manual(values = cp) +   
...
j.order <- unique(df1$journal[order(df1$N.sort, decreasing = FALSE)]) 
df1$journal <- factor(df1$journal, levels = j.order)




ggplot(na.omit(df1), aes(x = N, y= journal, group = journal, colour = studyType)) + 
...
ggplot(na.omit(df2), aes(x = Prop, y= journal, group = journal)) + 
...
page.plot <- ggplot(pages.df, aes(years, Mean.Pages, size = Std.Pages, group = 1))+ 
    geom_line(size=.75, colour =  "#5e3c99") +
    geom_line(aes(years, Median.Pages, group = 1), size = .75, colour = "#b2abd2", alpha = .70) + 
    geom_point(colour = "#5e3c99") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
    title = element_text(size = 10)) +
    scale_x_discrete(breaks=c(seq(1989, 2013, 2))) +
    ...
    ylab("Value") +
    xlab("Year") +
    labs(size = "Standard deviation", colour = "Median")  +
    ggtitle("Article Length by Year")  
  
```

```{r ratios}
years <- c(1989:2013)
article.grad.ratio <- articles.year.df$year.count / nsf$n.grads

ratio.df <- data.frame(cbind(years, article.grad.ratio))
colnames(ratio.df) <- c("years", "ratio")
ratio.df <- ratio.df[-(1:13), ]

grad.ratio.df.melted <- reshape2::melt(ratio.df, id.vars="years")



ratio.plot <- ggplot(grad.ratio.df.melted, aes(years, value, group= variable, 
    color=variable)) + geom_line() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
    title = element_text(size = 10), 
    legend.title=element_blank()) +
    scale_colour_manual(values=cbPalette)  +
    scale_x_continuous(breaks=c(seq(2002, 2013))) +
...
Data from the National Science Foundation regarding the number of earned doctorates in social work and social welfare were added to further contextualize the publication trends.  These data reveal both growth and decline in the number of earned doctorates over the period of time for which data were available.  The number of earned doctorates exhibited a rise from 2003 to 2005, which corresponded to a rise in the number of article published during these years.  Between 2008 and 2012, the number of doctoral degrees exhibited a decline, but the number of published articles continued to rise.     

A bubble plot was created to facilitate interpretation of these data (see Figure 2).  This involved creating a scatter plot, mapping each journal's total number of publications on the X axis and h-Index value on the Y axis.  Each journal was assigned a quartile rating for its h-Index, and those quartile values were mapped to four unique colors.  Finally, the number of years of representation in the study was mapped to the size of the point, or _bubble_.  Journals without an h-Index rating are not represented in this plot (N = XX).  A smoothed loess line and confidence interval were added to show the trend between the h-Index scores and number of articles published.  

The loess trend line shows a moderate association between the h-Index and number of articles published.  However, a number of notable deviations are observed.  Eleven journals were in the highest h-Index quartile.  The _British Journal of Social Work_ had the highest h-Index value but ranked third with regard to the overall number of publications. XXXX, XXXX, and XXXX were also in the top quartile but had comparably fewer published articles and existed for less time than the other journals in the same quartile. _Families in Society_ had the most published articles, spanning the full 25-year study period, but was in the second h-Index quartile.  

Figure 3 provides a descriptive summary of the number of authors per article over time.  As indicated by the median, it was more common for articles to be sole-authored until 2006, at which time the median fluctuated between one and two authors, and then stabilizing in 2009 at 2 authors.  The steady increase in the standard deviation since 2000 indicates an increase in the number of authors for each article. The length of articles also increased over time, starting at roughly 12.5 pages in 1989 and increasing steadily over time and stabilizing around 15 pages in 2013.  The standard deviation remained relatively stable over the 25 year period, with all values falling between the range of XX and XX.  

The length of the articles showed some change over time, going from approximately `r round(pages.df$Mean.Pages[pages.df$years == "1989"])` pages in 1989.  The length of the articles increased to about 15 pages and has remain at approximately this length since the early 2000's.  The standard deviation has remained fairly stable over the time period, ranging from `r options(digits=2); min(pages.df$Std.Pages)` to `r options(digits=2); max(pages.df$Std.Pages)`.  
