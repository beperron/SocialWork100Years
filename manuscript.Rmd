...
evidence.journals <- full_join(evidence.summary3, evidence.summaryOverall, by = "journal") %>%
    mutate(N.prop = N/Overall.N) %>% group_by(journal) %>%
    mutate(Overall.N.Strength = sum(N))
    
evidence.journalsOverall <- evidence.journals %>% group_by(journal) %>%
    mutate(Overall.N.Strength = sum(N))

journalOrder <- evidence.journalsOverall

ggplot(na.omit(evidence.journals), aes(N.prop, journal, group = journal, colour = studyType)) + geom_point(size = 3, alpha = .70) +
    theme_bw() +
    scale_colour_manual(values = cp) +
    facet_grid(.~studyType)

ggplot(na.omit(evidence.journals), aes(N, journal, group = journal, colour = studyType)) + 
