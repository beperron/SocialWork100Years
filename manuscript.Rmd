...
## Social work journal titles
 
The primary unit of analysis for this study was bibliographic records of social
...
To ensure comprehensive coverage, we also conducted a search of the Internet
and other bibliographic databases to identify other journals that may be
considered a disciplinary social work journal. The search terms included: 
...
Journal titles that were identified in the supplemental search were reviewed
by the study team. When disagreements occurred, the authors reviewed the
...
After the target list of social work journal titles was finalized, we attempted
to identify all article records from these journals using three major databases
on the EbscoHost platform (PscyINFO, Social Science Abstracts, and Social Work
Abstracts) and 46 minor databases on Proquest (e.g., ERIC, Sociological
...
In addition to the foregoing discrepancies, we also found other problems among
the extracted set of article records.  For example, the filtering of article
...
the final database, it was not feasible to manually inspect all records.  Thus,
we randomly sampled article records from the database and cross-checked journal
meta-data with information listed on the journal's web page.  To help build
confidence in the comprehensiveness of the constructed database, we obtained
aggregate publication data from Scopus and made systematic comparisons with the
database constructed for this project.  Scopus data was selected because it is
...
extraction process.  The systematic comparisons are  

## Reproducible research

In addition to post-processing original article records, R was also used for
all other data analysis.  R was an ideal analytic tool because it is
...
written report.  The source code and data extracts is also curated at openICPSR
...
practices of reproducible research (cite King), we have [BRYAN -- DESCRIBE HERE
HOW THE PS JOURNAL HANDLES THIS ISSUE ... ASSUME THE EDITOR IS ON-BOARD WITH
THIS APPROACH].
