...
hIndex <- read.csv("/Users/beperron/Git/SocialWorkResearch/Data/hIndex.csv")
hIndex$h5Index <- ifelse(is.na(hIndex$h5Index), 0, hIndex$h5Index)
hIndex$h5Median <- ifelse(is.na(hIndex$h5Median), 0, hIndex$h5Median)
...
```{r dataNSF, eval=TRUE}
n.grads <- c(rep(NA, 13), 238, 273, 304, 323, 308, 287, 338, 332,      
    308, 289, 280, NA)

years <- c(1989:2013)

nsf <- data.frame(cbind(n.grads,  years))
...
title <- sm.scopus.merged %>% arrange(desc(Overall.N)) %>% distinct(Abbrev) %>% filter(!is.na(Abbrev))
title.order = title$Abbrev
sm.scopus.merged$title2 <- factor(sm.scopus.merged$Abbrev, levels = (title.order))
Overall.N <- distinct(sm.scopus.merged, title2) 



hd.df <- sm.scopus.merged %>% select(Year, Abbrev, Journal, N, Overall.N, title2)


scopus.plot.df <- sm.scopus.merged %>% select(Year, Abbrev, Journal, Documents,Overall.N)
scopus.plot.df$Documents <- ifelse(scopus.plot.df$Documents == 0, NA, scopus.plot.df$Documents)
scopus.plot.df <- scopus.plot.df[!is.na(scopus.plot.df$Documents), ]
...
lab <- paste("N=", title$Overall.N, sep="")
Abbrev <- title$Abbrev

ldata <- data.frame(xpos, ypos, lab, Abbrev)

ggplot(hd.df,  aes(Year, N), group=1) + geom_line(colour="#e66101") +
    facet_wrap(~ Abbrev) + 
    geom_line(data = scopus.plot.df, aes(Year, Documents), colour="#5e3c99") + 
    theme_bw() +
...
ggplot(sm.scopus.merged, aes(X, Documents)) + geom_point(aes(colour = Journal)) + 
    facet_wrap(~Year) +
    geom_smooth()


cor(sm.scopus.merged$N, sm.scopus.merged$Documents, use ="complete.obs", method = "spearman")



sm.df$top10 <- ifelse(is.na(sm.df$top10), "No", sm.df$top10)


title <- sm.df %>% arrange(desc(Overall.N)) %>% distinct(title)
title.order <- title$title
sm.df$title2 <- factor(sm.df$title, levels = (title.order))
Overall.N <- distinct(sm.df, title)  



xpos = rep(12.5, 78)
ypos = rep(130, 78)
title2 <- unique(sm.df$title2)
lab <- paste("N=", Overall.N$Overall.N, sep="")


ldata <- data.frame(xpos, ypos, lab, title2)



ggplot(sm.df, aes(year, N, group=1)) + geom_line(aes(colour = top10)) + 
facet_wrap( ~ title2, ncol=7) +
theme_bw() +
...
ylim(0, 150) + 
geom_text(data = ldata, aes(x =xpos, y = ypos, label = lab), size =3) +
    scale_color_manual(values=cp[c(4,2)], 
                       name = "Top-10 Journal") + 
    ylab("") + 
    xlab("")
...
final.list.index <- full_join(final.list, hIndex, by = "title")

final.list.index$h5Index <- ifelse(is.na(final.list.index$h5Index), 0,final.list.index$h5Index)
#x <- ggplot(final.list.index, aes(x =reorder(title, n), y = n, colour = h5Index)) + geom_point(stat="identity") + coord_flip() + 
#    theme_bw() 
final.list.index$scaleScore <- scale(final.list.index$n)
final.list.index$hScaled <- scale(final.list.index$h5Index)
final.list.index <- arrange(final.list.index, scaleScore)


x <- ggplot(final.list.index, aes(x = scaleScore, y = title)) + geom_point(stat="identity", colour="red") + 
    xlim(c(-3, 3)) 
