...
ggplot(as.data.frame(xtabs(N~year+studyType, evidence.summary)), 
    ...
    scale_fill_manual(values=cp, 
        ...
        labels = c("Systematic review", "RCT", "Quasi-experimental", 
           "Meta-analysis")) +
  ylim(c(0, 60)) + 
...
levels(df$studyType) <- c("Meta-analysis", "Quasi-experimental", "Observational", "RCT", "Systematic review")

ggplot(na.omit(df), aes(year, studyProp, colour = studyType, 
    group = studyType), na.rm=TRUE) + 
    geom_smooth(aes(fill=studyType)) + facet_wrap(~studyType) + 
    scale_fill_manual(values = cp) +   
...
j.order <- unique(df$journal[order(df$N.sort, decreasing = FALSE)]) 
...
j.order <- unique(df2$journal[order(df2$Prop, decreasing = FALSE)]) 
df2$journal <- factor(df2$journal, levels = j.order)

ggplot(na.omit(df2), aes(x = Prop, y= journal, group = journal)) + 
    geom_segment(aes(yend = journal), xend=0, colour="#b2abd2") +
    geom_point(aes(size = Overall.N.Strength), colour = "#5e3c99") + 
    scale_color_manual(values = cp) + 
    theme_bw() +
    theme(panel.grid.major.y = element_blank())
