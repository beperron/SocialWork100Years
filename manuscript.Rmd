...
hIndex.df <- arrange(hIndex, desc(h5Median)) %>% select(title, h5Median)
hIndex.df$top10 <- NA
hIndex.df$top10[1:10] <- "yes"
hIndex.df$top10[11:nrow(hIndex.df)] <- "no"
hIndex.df <- data.frame(hIndex.df)
...
sm.df <- full_join(sm.df, hIndex.df, by = "title") %>% 
    ungroup %>% 
    select(-h5Median) 
...
x.df <- data.frame(title = Overall.N$title, label = title$Overall.N)

xpos = rep(10, 78)
ypos = rep(130, 78)
lab <- Overall.N$Overall.N
...
ylim(0, 150) + 
geom_text(data = ldata, aes(x =xpos, y = ypos, label = lab, size =1), parse=FALSE)
