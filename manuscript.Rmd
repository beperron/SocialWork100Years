...
```{r imageMaker, comment=NA, echo=FALSE, eval=TRUE}
# Read data files
...
n.grads <- c(rep(NA, 13), 238, 273, 304, 323, 308, 287, 338, 332,      
    308, 289, 280, NA)
n.grads.1 <- c(rep(NA, 14), 238, 273, 304, 323, 308, 287, 338, 332, 
    308, 289, 280)
n.grads.2 <- c(rep(NA, 15), 238, 273, 304, 323, 308, 287, 338, 332, 
    308, 289)

years <- c(1989:2013)

growth <- c(rep("Unknown", 14), rep("Increase", 2), rep("Decrease", 
    3), "Increase", rep("Decrease", 4), "Unknown")

nsf <- data.frame(cbind(n.grads, n.grads.1, n.grads.2, years))
colnames(nsf) <- c("n.grads", "n.grads.1", "n.grads.2", "years")
...
```{r finalJournalList}
final.list.updated <- final.list %>% mutate(study.time = 
    as.numeric(last)-as.numeric(first) +1)

final.list.25 <- final.list.updated %>% filter(study.time == 25)

journals.year <- left_join(n.so, n.yr) %>%
    group_by(title)

journals.2012 <- filter(journals.year, year == "2012") %>%
    group_by(title) %>%
    summarize(N = n())

journals.2012 <- arrange(journals.2012, desc(N))
journals.2012$cumS <- cumsum(journals.2012$N)
journals.2012$shortTitle <- factor(c(1:nrow(journals.2012)))

journals.2012$cumperc <- (1:nrow(journals.2012))/nrow(journals.2012) 
journals.2012 <- arrange(journals.2012, desc(N))

journals.2012 <- transform(journals.2012,
                          shortTitle = reorder(shortTitle, 
                                                order(N, decreasing = TRUE)))
ggplot(journals.2012, aes(x = title, y = N)) + 
...
plot.article.count <- ggplot(nsf, aes(as.factor(years), 
    ...
    annotate("point", x = as.factor(2003), y = 720, alpha = .60, color = "green", size = 6) + 
    annotate("point", x = as.factor(2015), y = 750, alpha = .60, color = "green", size = 6)  
...
plot.journal.count2 <- ggplot(n.journals.year, aes(as.factor(record.y), 
    y = Journal.count, group=1)) + 
    geom_line(colour="black") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8), 
    title = element_text(size = 10), plot.title=element_text(size=10)) + 
...
n_authors2$ymin <- n_authors2$Average - n_authors2$Standard_Deviation
n_authors2$ymax <- n_authors2$Average - n_authors2$Standard_Deviation


plot.author.count2 <- ggplot(n_authors2, aes(ID, y = Average, group=1, size = Standard_Deviation, colour=as.factor(Median))) + 
    geom_line(size=.75) +
    geom_point() + 
...
pages.df.melted <- reshape2::melt(pages.df, id.vars = "years")

```

```{r pages.plot, eval=TRUE}

page.plot <- ggplot(pages.df.melted, aes(years, value, group = 
...
page.plot <- ggplot(pages.df, aes(years, Mean.Pages, colour=as.factor(Median.Pages), size = Std.Pages, group = 1))+ 
    geom_line(size=.75) +
    geom_point() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
    title = element_text(size = 10), 
    legend.title=element_blank()) +
    scale_x_discrete(breaks=c(seq(1989, 2013, 2))) +
    scale_y_continuous(limits = c(10.5,16)) +
    ylab("Value") +
    xlab("Year") +
    ggtitle("Article Length by Year")
````
...
plot.PhD <- ggplot(nsf, aes(years, n.grads, group=1)) + 
    ...
    geom_text(aes(2002, 230, label = "238", colour = "black"))
```
