...
article.grad.ratio.1 <- articles.year.df$year.count / nsf$n.grads.1
article.grad.ratio.2 <- articles.year.df$year.count / nsf$n.grads.2


ratio.df <- data.frame(cbind(years, article.grad.ratio, article.grad.ratio.1, article.grad.ratio.2))
colnames(ratio.df) <- c("years", "ratio", "ratio.1", "ratio.2")
...
The study was restricted to a 25-year timeframe, 1989 (inclusive) to 2013 (inclusive).  Table 1 provides a summary of all the journals contained in the database, the years of inclusion, number of articles present, and whether the journal was included on the list of disciplinary journals defined by Hodge and Lacasse (2011).  Appendix A provides a summary of journals on the list of Hodge and Lacasse that were not included in this historical database (N = `r length(hodge.swHistory.diff)`).  A total of `r nrow(fewArticles)` journals were excluded because they had only 1 or 2 article records (see Appendix A).  Otherwise, all other journals met the minimum requirement of at least 20 article records. It is important to note that the years of inclusion in the database may not be fully consistent with the actual output of the journal for the given timeframe.  The possible discrepancies are accounted due to delays and errors of indexing, which is a known and longstanding problem in social work research (see Holden; Holden).  The problems associated with indexing are reviewed in further detail in the discussion section of this article.  

`r nrow(final.list.25)` journals were represented across all 25 years.  The mean number of years of inclusion is `r options(digits=2); mean(final.list.updated$study.time)` `r options(digits=2); median(final.list.updated$study.time); sd(final.list.updated$study.time)`.  Figure 1a provides a graphical summary of the number of journal titles for each year over the 25 year study period.  The first year of the study period contained XX journals, and the maximum number of articles was observed for the years XX.  The decline in years 2012 to 2013 may be an artifact due to indexing, given that some journals were still in existence but article records could not be obtained (e.g., XXXX).


### Article summary

The historical database includes `r pretty(sum(articles.year.df$year.count))` overr the 25 year period (see Table 1).  The average number of articles per journal was `r options(digits=2); pretty(mean(articles.year.df$year.count))` (median = `r options(digits=2); pretty(median(articles.year.df$year.count))`, sd = `r options(digits=2); pretty(sd(articles.year.df$year.count))`).  The ratio of journals to articles (see figure 1a) was relatively consistent across time.  
...
The mean number of authors showed a rising trajectory over the study time period, with an average of `r options(digits=2); n_authors2$Average[n_authors2$ID == "1989"]` authors (`r options(digits=2); n_authors2$Standard_DeviationA[n_authors2$ID == "1989"]`) in 1989 to `r options(digits=2); n_authors2$Average[n_authors2$ID == "2013"]` authors (sd = `r options(digits=2); n_authors2$Standard_Deviation[n_authors2$ID == "2013"]`) in 2013.  The median number of authors was XX.  Between years XX and XX, the number fluctuated between 1 and 2, and has remained stable at 2 authors per year since 2009.  The standard deviation of the number of authors per paper has continued on a steady upward trajectory since the beginning of the study period.  

For further contextual information, the number of PhD degrees was plotted using data available from the NSF for years 2002 to 2012.  The ratio of the number of PhD degress to the number of articles published that year was XXXX.  
