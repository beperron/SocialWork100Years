...
plot.journal.count <- ggplot(n.journals.year, aes(record.y, 
    ...
    xlab("Year") + 
    ylab("Number of Unique Journal Titles") + 
...
evidence.df$studyType <- factor(evidence.df$studyType, labels= c("Meta-analysis", "Quasi-experimental", "RCT"))
...
journal.evidence.plot <- ggplot(na.omit(df1.reduced), aes(x = Abbrev, y = N)) + 
    ...
    ylim(c(0, 150)) +
    scale_fill_manual(values = cp) +
    theme(legend.position = c(.65,.7),  legend.title=element_blank(), legend.background = element_rect(fill="transparent"))
```
...
propPlot <- ggplot(proportionEBP3.df, aes(journalProp, Abbrev)) +
    ...
    ylab(NULL) +
    xlab("Proportion") +
    geom_text(aes(x = journalProp + .07, y = Abbrev, label = journalProp), size = 3) +
    xlim(c(0, .5))  
