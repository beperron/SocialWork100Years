...
```{r Figure 1 Small Multiples, cache=TRUE}
...
DataBase <- rep("Scopus", nrow(scopus.df))
scopus.df$DataBase <- DataBase
scopus.df$Documents <- ifelse(scopus.df$Documents == 0, NA, scopus.df$Documents)
...
journalOutput <- distinct(sm.scopus.merged, Journal) 

ann_scopus <- select(journalOutput, Journal, Overall.N)
ann_scopus <- ann_scopus[1:9, ]
ann_scopus$label <- paste0("N=", pretty(ann_scopus$Overall.N))
...
ggplot(combined_reduced,  aes(Year, Documents, colour = DataBase)) +
  ...
  xlab("\nYear") +
     annotate("segment", x = 1992, xend = 1995, y = 190, yend = 190, colour = cp[2], size = 1, alpha = .80) + 
     annotate("segment", x = 1992, xend = 1995, y = 170, yend = 170, colour = cp[4], size = 1, alpha = .80) +
   geom_text(data = ann_scopus, aes(x = 1999, y =  170, label = label), colour = "black", size = 4)
...
# 
 ...
 xpos = rep(1999, 79)
 ypos = rep(180, 79)
 ldata <- data.frame(xpos, ypos, lab, Overall.N$Journal) 
 
 ldata <- na.omit(ldata)
 
 
 titles.reduced <- unique(sm.scopus.merged$Journal)
 titles.reduced.a <- titles.reduced[1:8]
...
# Plotting function    
# 
reduced.ldata <- ldata[1:8, ]

reduced.ldata$Abbrev <- factor(reduced.ldata$Abbrev, levels = c("Families in Society", "Social Work", "British Journal of Social Work", "Journal of Gerontological Social Work", "Research on Social Work Practice", "Social Work in Health Care", "International Social Work", "Health & Social Work") )

reduced.ldata$Abbrev = c("Families in Society", "Social Work", "British Journal of Social Work", "Journal of Gerontological Social Work", "Research on Social Work Practice", "Social Work in Health Care", "International Social Work", "Health & Social Work")
reduced.smallmult.df <- sm.scopus.merged
reduced.smallmult.df$Abbrev <- reduced.smallmult.df$Journal
reduced.smallmult.df <- na.omit(reduced.smallmult.df)

reduced.smallmult.df$Abbrev <- as.factor(reduced.smallmult.df$Abbrev)
reduced.smallmult.df$Abbrev <- factor(reduced.smallmult.df$Abbrev, levels = c("Families in Society", "Social Work", "British Journal of Social Work", "Journal of Gerontological Social Work", "Research on Social Work Practice", "Social Work in Health Care", "International Social Work", "Health & Social Work") )

new.data <- reduced.smallmult.df[reduced.smallmult.df$Abbrev %in% titles.reduced.a, ]
new.data

 plotSmallMultiples.a <-  ggplot(new.data,  aes(Year, N)) +
     geom_line(colour="#e66101") +
     geom_line(data = reduced.smallmult.df[reduced.smallmult.df$Journal %in% titles.reduced.a, ], 
               aes(Year, Documents), colour = "#533c99", alpha = .80) +
   
     facet_wrap(~ Abbrev) + 
    geom_text(data = reduced.ldata, aes(x =xpos, y = ypos, label = lab), size = 5) +
     theme_bw() +
     ...
     annotate("segment", x = 1992, xend = 1995, y = 190, yend = 190, colour = "#e66101") + 
     annotate("segment", x = 1992, xend = 1995, y = 170, yend = 170, colour = "#533c99") 
     #ggtitle("Figure 1. Comparison of Historical Database with Scopus\n for Journals with Highest Output (1989 - 2013)") + 
     # theme(plot.title = element_text(size = 19))
     # 
#ggsave(file = "scopusComparison.png", plotSmallMultiples.a)
 ...
 plotSmallMultiples.1 <-  ggplot(sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.1, ],  aes(Year, N)) +
     ...
     panel.grid.minor = element_blank(), 
     axis.text.x = element_text(angle=45, hjust=1, size = 10)) + 
     geom_text(data = ldata[1:40, ], aes(x =xpos, y = ypos, label = lab), size = 4) +
     ylim(c(0,225)) +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) +
     annotate("segment", x = 1989, xend = 1992, y = 215, yend = 215, colour = "#e66101") + 
     annotate("segment", x = 1989, xend = 1992, y = 150, yend = 150, colour = "#533c99") +
...
plotSmallMultiples.2 <- ggplot(sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.2, ],  
     ...
     panel.grid.minor = element_blank(), 
     axis.text.x = element_text(angle=45, hjust=1, size =10)) + 
     geom_text(data = ldata[41:79, ], aes(x =xpos, y = ypos, label = lab), size =4) +
     ylim(c(0,225)) +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) +
     annotate("segment", x = 1989, xend = 1992, y = 215, yend = 215, colour = "#e66101") + 
