...
exp.07 <- c(737, rep(NA, 24)) 
for(i in 2:25){
    exp.07[i] <- exp.07[i-1] + exp.07[i-1]*.05
}

exp.175 <- c(737, rep(NA, 24))
for(i in 2:25){
       exp.175[i] <- exp.175[i-1] + exp.175[i-1]*.175
    }


exp.171 <- c(737, rep(NA, 24))
for(i in 2:25){
       exp.171[i] <- exp.171[i-1] + exp.171[i-1]*.171
    }


articles.df$exp.175 <- exp.175
articles.df$exp.07  <- exp.07
articles.df$exp.171 <- exp.171

ggplot(articles.df, aes(Year, exp.171, group =1)) + 
    theme_bw() + 
    geom_point(colour=cp[2], size = 3, alpha=.80) +
    geom_line(colour=cp[2]) + 
    theme(axis.text.x = element_text(size=8), 
    title = element_text(size = 10), plot.title=element_text(size=10)) + 
    xlab("Year") + 
    ylab("Articles") + 
    ...
    #scale_y_continuous(breaks = c(seq(0, 2200, 250)), limits= c(500, 2100)) +
    theme(axis.title.x = element_text(vjust=-0.5), axis.title.y = element_text(vjust=.75)) +
    geom_line(aes(Year, CumSum), colour = cp[3]) +
    geom_point(aes(Year, CumSum), colour = cp[3]) 
...
CAGR <- function(start, finish, periods){
  ...
  # Returns the Compound Annual Growth Rate
    cagr <- round(((finish/start)^(1 /periods) - 1)*100, digits=2)    
  return(cagr)
}

setwd("C:/Users/beperron/Git/SocialWorkResearch/Data")
...
plot.cagr.ss <- ggplot(cagr, aes(x=field, y=cagr, fill=field, group =1)) + 
  theme_bw() + 
  geom_bar(width=0.8, stat='identity') +
  theme(axis.text.x = element_text(size=8), 
        title = element_text(size = 10), plot.title=element_text(size=10)) + 
  xlab("Field") + 
  ylab("Cumulative annual growth rate (%)") + 
  ggtitle("Cumulative annual growth rates for article production across the social sciences, 1989-1998") +
  theme(plot.title = element_text(size = rel(1.5))) +
  scale_y_continuous(breaks = c(seq(0, 40, 5)), limits=c(0, 40)) +
  theme(axis.title.x = element_text(vjust=-0.5), axis.title.y = element_text(vjust=.75))+
  scale_fill_manual(values=c("#b2abd2", "#b2abd2", "#fdb863", "#b2abd2", "#b2abd2", "#b2abd2", "#b2abd2"))+
  theme(legend.position="none") +
  scale_x_discrete(labels=c("Psychology","Economics","Social Work","Sociology","Political Science", "History", "Anthropology")) +
