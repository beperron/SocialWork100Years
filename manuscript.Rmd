...
# Plotting function    
 plotSmallMultiples.1 <-  ggplot(sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.1, ],  aes(Year, N)) +
     ...
     geom_line(data = sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.1, ], aes(Year, Documents)) + 
     facet_wrap(~ Abbrev) + 
     theme_bw() +
     ...
     geom_text(data = ldata[1:40, ], aes(x =xpos, y = ypos, label = lab), size =3) +
     ylim(c(0,225)) +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) 

plotSmallMultiples.2 <- ggplot(sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.2, ],  aes(Year, N)) +
     geom_line(colour="#e66101") +
     geom_line(data = sm.scopus.merged[sm.scopus.merged$Journal %in% titles.reduced.2, ], aes(Year, Documents)) + 
     facet_wrap(~ Abbrev) + 
     theme_bw() +
     theme(strip.text.x = element_text(size = 8)) +  
     theme(legend.position = "bottom", 
     panel.grid.minor = element_blank(), 
     axis.text.x = element_text(angle=45, hjust=1, size =8)) + 
     geom_text(data = ldata[41:79, ], aes(x =xpos, y = ypos, label = lab), size =3) +
     ylim(c(0,225)) +
     scale_x_continuous(breaks=seq(1989, 2015, 8)) 
...
df.96.10 <- df.96.10 %>% mutate(countNotCited = HD.N * propNotCited)
...
# We need to include years before and after the scopus data.  
...
#============================================
# Compute percentage variable and melt
#============================================

uncited.df$percentage <- uncited.df$Uncited/uncited.df$Published*100    

uncited.melted <- reshape2::melt(uncited.df[,c("Year", "Published", "Uncited")], 
     id.vars = c("Year"))
...
plotArticles <- ggplot(uncited.melted, aes(Year, value, colour = variable,)) + 
    geom_line(alpha = .60) + 
    geom_point(size = 4) +
...
boxplot.scopus <- ggplot(subset(scopus.df, subset =Year <=2013 & Year >= 1999), aes(as.factor(Year), IPP)) +
    geom_boxplot()
...
stacked.plot <- ggplot(as.data.frame(xtabs(N~year+studyType, evidence.summary)), 
...
journal.evidence.plot <- ggplot(na.omit(df1), aes(x = N, y= journal, group = journal, colour = studyType)) + 
...
A total of XX article records from XX unique journal titles met the inclusionary
criteria and were included in the historical database.  

Figure 1 uses a
small multiples plot to summarize the number of article records for each year
throughout the study period.  Small multiples, also referred to as trellis
and lattice charts, are the best design solution for visually enforcing comparisons of
change (Tufte, XXXX).  In this context, we use two different data sources to show differences in publishing across the journals -- the orange line represents data from the SWHD, and the purple line represents data from Scopus.      


the small multiples to show differences in publishing across the journals, 

show differences in publication history for each journal.  


The number of article records for each year (by journal) is denoted by an orange line, along with the overall number of article records.  For further contextual information, the number of article records available from Scopus is presented with a purple line.   
This plot reveals variability in publishing over time and across journals.  The for each journal and across journals.  
First, the historical database indexed XX journals, whereas Scopus indexed only XX.  For the number of journals indexed by Scopus, the number of article records tend to be higher than the number of records available in the historical database.  The primary reason for this difference is that Scopus included editorials and book reviews in their summary data, whereas these were excluded from the SWHD.  Other difference can be attributed to errors and gaps in indexing across both databases.  As we relied upon aggregate data from Scopus, we were unable to quantify these differences.  



should be noted that the number of article records available from Scopus tends to be greater than the historical database because Scopus included editorials (AND REVIEWS), whereas these were excluded from the historical database.  




According to Tufte (Evisioning information), small multiples are the best design s
