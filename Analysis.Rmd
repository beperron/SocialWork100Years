...
select.keywords <- c("power", "oppression", "discrimination", "socialjustice", "qualitativeresearch")
select.keywords.df <- kp.year %>% filter(keyword %in% select.keywords)
...
ggplot(location.year.top, aes(x = year, y = n, colour = country )) + geom_line() +
    ggtitle("Number of articles by Top 5 Countries")    
...
ggplot(location.year.top.no.us, aes(x = year, y = n, colour = country )) + geom_line() +
    ggtitle("Number of article by Top 5 Countries (excluding United States)")
```
...
n_authors.1t <- n_authors.1 %>% mutate(n = ifelse(n >=13, 13, n))




ggplot(n_authors.1t, aes(x=as.factor(n))) + 
    geom_bar(fill="white", colour="black") + 
    #stat_bin(binwidth=.5) +
    ggtitle("Number of Authors Listed Per Article") + 
    xlab("Author") +
    ylab("Number of Articles") +
    scale_y_continuous(limits = c(0, 15000), breaks = seq(0, 14000, 2000)) + 
    scale_x_discrete(limits = c(as.character(1:12), "13+"))
