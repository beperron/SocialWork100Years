...
+ Article record retrieval date: January 24, 2015
Database: psycInfo, Social Science Abstracts, Social Work Abstracts (via EbscoHost Platform) 

+ Primary search:  Hodge's list

+ Supplemental search:  SO "social work" or SO "social welfare" or SO "social casework" or "social case work" or SO "social service" or "social services" or SO "human service" or SO "human services" or SO "social development" or SO "social environment" 
...
+ Results filtered to exclude years 2014 and 2015 (due to delays in indexing)
...
bwr.df$record <- ifelse(bwr.df$record == "Computers in Human Services", 
                       "Journal of Technology in the Human Services", 
                        bwr.df$record)
...
filter(bwr.df, attributes == "SO") %>% group_by(record) %>% arrange(record) %>% summarise(Article.Count = n())

filter(bwr.df, attributes == "TI") %>% summarize(N = n())
```
