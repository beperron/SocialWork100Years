...
Data collection for this study was separated into three phases.  

## Search Query

The first phases involved querying three different databases serviced by the EbscoHost web platform: psycInfo, Social Science Abstract, and Social Work Abstracts.  These databases were selected because they provide the indexing services for the majority of social work journals.  Additionally, search queries from these databases, served by EbscoHost, allowed search results to be exported raw text (*.txt) documents, referred to as a _generic bibliographic format_.  Other databases and search engines (e.g., Google Scholar) were not queried because the results could not be saved in a common format.  This is a limitation of the study that is covered in the discussion section.  

The search began by querying each database for every _journal article_ from every journal that was included in the list by Hodge and ... [Describe this list] ... Each query was performed independently on each database, 





We had a three-level search process designed to be as comprehensive as possible.  First, we began by running a separate search query on each database for every journal contained in the list by Hodge.  We then conducted a subsequent search of all journal titles based on the following key words:
...
In each query, results were limited to only journal articles, which excluded other document types including (but not limited to) editorials, book reviews, software reviews, and obituaries.  All searches were conducted between January 24, 2015 and January 30, 2015. Results were subsequently filtered to exlclude years 2014 and 2015 due to delays in indexing.  Search results were exported from EbscoHost platfrom into a _generic bibliographic format_.  This is an unstructured text file (*.txt) that was processed using in R.  The multiple searches were then transformed into an analyzable database using a customized script developed by the first two authors of the study (BEP and BGV). 

Study authors reviewed all journal titles and 



# Data Pre-Processing

# NOTE: CONFIRM THAT SOCIAL CASEWORK IS SAME AS FAMILIES IN SOCIETY.  ENSURE WE 
# HAVE THE ARTICLES FOR WHEN THE JOURNAL WAS ALSO CALLED THE FAMILY
...
#save(bwr.df, file = "HistoricalArticles.R")
```

#add no authorship indicated
#Anonymous
x.temp <- filter(bwr.df, attributes == "article")
x.temp2 <- filter(bwr.df, attributes == "journal")
x.temp3 <- filter(bwr.df, attributes == "abstract")

dim(x.temp); dim(x.temp2); dim(x.temp3)
...
x.temp3 <- filter(bwr.df, attributes == "author")
b <- grepl("@", x.temp3$record)

test.case <- filter(bwr.df, attributes == "AU")
auth.name <- filter(test.case, grepl("(Perron,\\sB)", record))


x.temp <- filter(bwr.df, attributes == "KW" | attributes == "KP")
...
load("/Users/beperron/Git/SocialWorkResearch/Data/HistoricalArticles.R")
...
length(which(bwr.df$attributes == "TI"))
```

# Exclude years 2014 and 2015  

```{r}
years <- c("2014", "2015")
ebsco.remove <- bwr.df[bwr.df$record %in% years, ]
ebsco.remove <- ebsco.remove[, "articleID"]
bwr.df <- bwr.df[!(bwr.df$articleID %in% ebsco.remove),]
```



# Merge journal titles

```{r}
bwr.df$record <- ifelse(bwr.df$record == "Eugenics and Social Welfare Bulletin", 
                       "Eugenics & Social Welfare Bull.", 
                        bwr.df$record)

bwr.df$record <- ifelse(bwr.df$record == "The British Journal of Social Work", 
                       "British Journal of Social Work", 
                        bwr.df$record)


bwr.df$record <- ifelse(bwr.df$record == "Social Work & Social Sciences Review", 
                       "Social Work and Social Sciences Review", 
                        bwr.df$record)

bwr.df$record <- ifelse(bwr.df$record == "Social Work with Groups", 
                       "Social Work With Groups", 
                        bwr.df$record)

bwr.df$record <- ifelse(bwr.df$record == "Computers in Human Services", 
                       "Journal of Technology in Human Services", 
                        bwr.df$record)

bwr.df$record <- ifelse(bwr.df$record == "Journal of Sociology & Social Welfare", 
                       "Journal of Sociology and Social Welfare", 
                        bwr.df$record)

bwr.df$record <- ifelse(bwr.df$record == "The Family", 
                       "Families in Society",
                       ifelse(bwr.df$record == "Journal of Social Casework",
                       "Families in Society",
                       ifelse(bwr.df$record == "Social Casework",
                       "Families in Society",
                        bwr.df$record)))

bwr.df$record <- ifelse(bwr.df$record == "Social Work in Education", 
                       "Children & Schools", 
                        bwr.df$record)


#Note leave space after the (Electronic only)
bwr.df$record <- ifelse(bwr.df$record == "Contemporary Rural Social Work (Electronic only) ", 
                        "Contemporary Rural Social Work (Electronic only) ", bwr.df$record) 

```


# Journals excluded from search results.  
...
exclusions <- c(
    ...
    "University of Chicago Social Service Monograph")
ebsco.remove <- bwr.df[bwr.df$record %in% exclusions, ]
ebsco.remove <- ebsco.remove[, "articleID"]
bwr.df <- bwr.df[!(bwr.df$articleID %in% ebsco.remove),]
```

```{r unique_titles, echo=FALSE}
unique.titles <- filter(bwr.df, attributes == "SO") %>%
    group_by(record) %>%
    summarise(
        N = n())
```
...
title <- bwr.df %>%
        ...
        select(id = articleID, SO = record)

article <- bwr.df %>%
        filter(attributes == "TI") %>%
...
n.so.yr <- filter(bwr.df, attributes == "SO" | attributes == "YR")

n.so <- filter(bwr.df, attributes == "SO") %>% mutate(title = record) %>% 
        select(-attributes, -record)

n.yr <- filter(bwr.df, attributes == "YR") %>% mutate(year = record ) %>% 
        select(-attributes, -record)
...
n.articles.year <- filter(bwr.df, attributes == "YR") 
...
kp.df <- filter(bwr.df, attributes == "KP")
...
kp.yr <- filter(bwr.df, attributes == "YR") %>%
...
LO.df <- filter(bwr.df, attributes == "LO") %>%
         group_by(record) %>%
...
year <- filter(bwr.df, attributes == "YR") %>%
    mutate(year = as.numeric(record)) %>% select(-record, -attributes)

location.temp <- bwr.df %>%
